...
<body>
    ...
    <div class="w-[90%] sm:max-w-7xl mx-auto pt-20 pb-14 mt-7">
        ...
        <div class="bg-gray-100 pt-20 pb-14 mt-7">
            ...
            if ($userID !== null) {
                ...
                // Fallback to using IP address if user is not logged in
                $stmt = $connection->prepare("SELECT cart_items.*, 
                    products.name AS product_name, 
                    products.description AS description, 
                    products.currency_code AS currency_code, 
                        ...
                        <div class="mt-6 h-full md:mt-0 md:w-1/3">
                            <div class="rounded-lg border bg-white p-6 shadow-md ">
                                <div class="mb-2 flex justify-between">
                                    ...
                                    <p id="subtotalPrice"
                                    class="text-gray-700"><span class="mr-2"><?php echo $item['currency_code']; ?></span><?php echo number_format($totalPrice, 2); ?></p>
                                </div>
                                ...
                                <div class="flex justify-between">
                                    ...
                                    <div class="">
                                        <p id="totalPrice" class="mb-1 text-lg font-bold">
                                            <?php
                                            if ($item['currency_code'] === "BDT") {
                                                echo '<span class=" text-lg">৳</span>';
                                            } elseif ($item['currency_code'] === "USD") {
                                                echo '$';
                                            } else {
                                                echo '৳';
                                            }
                                            ?>
                                            <?php echo number_format($totalPrice, 2); ?> <span><?php echo $item['currency_code']; ?></span></p>
                                    </div>
                                </div>
                            ...
                            <div class="py-3 sm:px-6 sm:flex sm:flex-row-reverse justify-center w-full mt-5">
                                ...
                                class="w-full text-center block rounded-md  px-4 py-2 bg-blue-500 text-base font-medium text-white hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 sm:ml-3 sm:text-sm">
                                    Continue Shopping
                                </a>
                            </div>
                        </div>
    ...
    if (<?php echo json_encode($cartItems); ?>.length === 0) {
